...
class FollowingTableViewCell: UITableViewCell {
    ...
    var otherFollowingVC: OtherFollowingViewController?
    var user: User? {
        didSet {
            setupUserInfo()
        }
    ...
    @objc func unFollowAction() {
        if user?.isFollowing == true {
            ...
            followingVC?.present(alert,animated: true,completion: nil)
            otherFollowingVC?.present(alert,animated: true,completion: nil)
        }
    }
    
    @objc func toOtherVC() {
        if let id = user?.id {
            followingVC?.performSegue(withIdentifier: "OtherVC", sender: id)
            otherFollowingVC?.performSegue(withIdentifier: "OtherVC", sender: id)
        }
    }
